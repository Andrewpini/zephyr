...
static uint8_t elem_offset1[2] = {1, 2};
static uint8_t elem_offset2[3] = {4, 5, 6};
static uint8_t additional_data[2] = {100, 200};

static struct bt_mesh_comp2_record comp_rec[2] = {
	{.id = 1,
	 .version.x = 2,
	 .version.y = 3,
	 .version.z = 4,
	...
	{.id = 10,
	 ...
	 .additional_data_len = sizeof(additional_data),
	 .data = additional_data},
};

static struct bt_mesh_comp2 comp_p2_1 = {.record_cnt = 1, .rec = comp_rec};
static struct bt_mesh_comp2 comp_p2_2 = {.record_cnt = 2, .rec = comp_rec};
...
static void device_pb_remote_server_setup_unproved(const struct bt_mesh_comp *comp,
						   struct bt_mesh_comp2 *comp_p2)
{
	device_pb_remote_server_setup(comp, true);
	bt_mesh_comp_p2_reg(comp_p2);
}
